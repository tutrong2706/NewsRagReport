...
def truncate_code_blocks(content, max_lines=15):
    """Truncates or removes markdown code blocks."""
    def replacer(match):
        language = match.group(1)
        code = match.group(2)
        lines = code.split("\n")
        # code blocks often have a trailing empty line before the closing ```
        if len(lines) > 0 and lines[-1] == "":
            lines = lines[:-1]
        
        if len(lines) > 10:
            return "\n\n*(Mã nguồn chi tiết đã được lược bỏ để báo cáo ngắn gọn. Vui lòng xem trong source code dự án)*\n\n"
...
def preprocess_markdown(content, meta=None):
    ...
    content = re.sub(r"!\[([^\]]*)\]\(/NewsRagReport/images/", r"![\1](", content)
    ...
    content = truncate_code_blocks(content, max_lines=15)
