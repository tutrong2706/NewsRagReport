...
## 1. Unit & Integration Tests (Cục bộ)
...
| Metric | Đo lường gì | Target |
|--------|-------------|--------|
| **Faithfulness** | Câu trả lời có dựa trên ngữ cảnh không | > 0.8 |
| **Answer Relevancy** | Câu trả lời có trả lời câu hỏi không | > 0.8 |
| **Context Precision** | Chunks liên quan có được rank cao không | > 0.7 |
| **Context Recall** | Có lấy đủ các chunks liên quan không | > 0.7 |
...
# Dùng Groq-compatible endpoint cho evaluation
os.environ["OPENAI_API_KEY"] = os.getenv("GROQ_API_KEY")
os.environ["OPENAI_API_BASE"] = "https://api.groq.com/openai/v1"

# Initialize RAGAS với Groq LLM
...
def run_ragas_evaluation():
    """Chạy RAGAS evaluation trên pipeline hiện tại."""
    pipeline = Pipeline()
    results = []
...
| Low Score | Action |
|-----------|--------|
| Faithfulness | Cải thiện prompt: "Chỉ trả lời dựa trên ngữ cảnh, không bịa đặt" |
| Answer Relevancy | Thêm "Trả lời trực tiếp, ngắn gọn" vào prompt |
| Context Precision | Tune HNSW `ef_search`, tăng top-K, thêm metadata filters |
...
```python
# Trong Lambda functions - dùng structured JSON logs
import logging
import json
...
**Tiếp theo:** [Tối ưu chi phí](5.16-Cost/)