...
```
EventBridge Scheduler (02:00 UTC hàng ngày)
       │
       ▼
┌─────────────────┐
...
| Aspect | Lựa chọn | Lý do |
|--------|----------|-------|
| **Embedding Model** | `amazon.titan-embed-text-v2:0` | 1024 dims, AWS native, serverless, consistent với RAG query |
| **Chunk Size** | ~500 tokens, 50 overlap | Cân bằng context vs. precision |
| **Vector Index** | HNSW (m=16, ef_construction=64) | Fast approximate nearest neighbor |
| **Batch Size** | 50 articles per invocation | Giữ trong 15-min timeout |
| **Trigger** | EventBridge cron(0 2 * * ? *) | Hàng ngày sau crawler (01:00 UTC) |
...
# Bedrock client với retry config
...
def embed_text(text: str) -> list[float]:
    """
    Tạo embedding dùng Titan Embed v2.
    Returns list 1024 floats.
    """
...
def init_schema(conn):
    """Đảm bảo tables và pgvector extension tồn tại."""
...
def process_batch(cur, articles: List[dict], batch_limit: int = 50) -> int:
    """Xử lý batch bài viết thô. Returns count processed."""
    processed = 0
...
**Tiếp theo:** [RAG API (Lambda + API Gateway)](5.13-RAG-API/)